...
proximitydf_Aug2015<-filter(proximitydf[proximitydf$Date >= "2015-08-01" & proximitydf$Date <= "2015-08-31",])
head(proximitydf_Aug2015)

#before creating matrix need to rerun code for each month, and instead of editing the function, just use this generic df
proximitydf_date<-proximitydf_Aug2015
...
Proximity10<-function (animal1, animal2){
  ...
  matches10a<-grepl(animal2, focal1[,11])
  ten_a<-length(matches10a[matches10a==TRUE])
  
  matches5a<-grepl(animal2, focal1[,10])
  five_a<-length(matches5a[matches5a==TRUE])
  ...
  scanswithin10a<-sum(ten_a, five_a, two_a, one_a, contact_a)
  scanswithin10a
  ...
  matches10b<-grepl(animal1, focal2[,11])
  ten_b<-length(matches10b[matches10b==TRUE])
  
  matches5b<-grepl(animal1, focal2[,10])
  five_b<-length(matches5b[matches5b==TRUE])
  ...
  scanswithin10b<-sum(ten_b, five_b, two_b, one_b, contact_b)
  scanswithin10b
  
  out<-(scanswithin10a + scanswithin10b)/(totalscansasfocal1 + totalscansasfocal2)
  return(out)
}
...
####Long list of AIs so that we can put in LMM 
f<-NULL
for (i in 1:length(individuals)){
  for (j in 1:length(individuals)){
    f1 <-paste(individuals[i])
    f2 <-paste(individuals[j])
    AI_10 <- Proximity10(f1,f2)
    AI_2<-Proximity2(f1,f2)
    g<-cbind(f1,f2,AI_2,AI_10)
    f <- rbind(f,g)
  }}
head(f)
...
write.table(m, file="C:/Users/Kelsey/Box Sync/Cleaned data/AAPA2017/MOnthlyMatrices/Prox10Matrix_Aug2015.csv",row.names=TRUE,col.names=TRUE,sep=",")

write.table(f, file="C:/Users/Kelsey/Box Sync/Cleaned data/AAPA2017/MonthlyAIsforLMM/Prox10Long_Aug2015.csv",row.names=TRUE,col.names=TRUE,sep=",")
...
write.table(totscans, file="C:/Users/Kelsey/Box Sync/Cleaned data/AAPA2017/MonthlyMatrices/FocalTotalScans_Aug2015.csv",row.names=TRUE,col.names=TRUE,sep=",")
...
write.table(m, file="C:/Users/Kelsey/Box Sync/Cleaned data/AAPA2017/MonthlyMatrices/Prox2Matrix_Aug2015.csv",row.names=TRUE,col.names=TRUE,sep=",")
